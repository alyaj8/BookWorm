...
export default function FriendProfile({ navigation, route }) {
  ...
  console.log(
    follower,
    follower.join("").includes(auth.currentUser.uid),
    "nkgnkkkkkkkkk"
  );
    ...
    >
      ...
      <View style={{ width: "102%", alignItems: "center" }}>
        <TouchableOpacity
          onPress={() => onFollow()}
          disabled={
            follower.join("").includes(auth.currentUser.uid) ? true : false
          }
        >
          <View
            style={{
              alignSelf: "center",
              alignItems: "center",
              backgroundColor: follower.join("").includes(auth.currentUser.uid)
                ? "gray"
                : "#00a46c",
